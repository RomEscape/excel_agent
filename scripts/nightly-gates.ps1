...
#       reports = $env:OFFICE_CLAW_REPORTS_DIR, 없으면 %LOCALAPPDATA%\office_claw\reports.
...
# 산출물 폴더 — 파이썬(office_claw_sidecar.config.get_reports_dir)과 같은 규칙. 없으면 만든다.
$Reports = $env:OFFICE_CLAW_REPORTS_DIR
if (-not $Reports) { $Reports = Join-Path $env:LOCALAPPDATA 'office_claw\reports' }
New-Item -ItemType Directory -Force -Path (Join-Path $Reports 'nightly') | Out-Null
...
if ($Register) {
  ...
  Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings `
    -Description "야간 게이트: pytest + 파괴 게이트 72 + 말투 게이트 624. 결과는 $Reports\nightly\LATEST.md" -Force | Out-Null
