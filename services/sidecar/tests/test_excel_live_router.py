...
# ═══ 이하: dev 병합분(2026-08-29) ═══
# dev의 chat-tools 루프(run_excel_tool_turn) 전용 테스트 8건은 가져오지 않았다 —
# 이 브랜치는 그 루프를 3단 해석(규칙→의도→플래너) + 실행 검증 루프로 대체했다.
# 감사 로그(command_log) 배선은 아래 "작업 기록 감사 로그" 핀들이 이 브랜치의
# 흐름(명령 경로 승인 대기·재개)에 맞춰 다시 검증한다.
...
# ── 작업 기록 감사 로그(command_log) — dev 배선을 이 브랜치 흐름으로 검증 ──────
...
def test_confirm_command_logs_once_and_approval_updates_row(monkeypatch):
    """승인 대기 1행 → 승인 시 같은 행 갱신, SAFE 중복 기록 없음."""
    monkeypatch.setattr(excel_live_router, "get_excel_live_service", _one_fake())
    ...
    body = first.json()
    assert body["approval_required"] is True
    confirm_rows = [r for r in audit.logged if r["grade"] == "CONFIRM"]
    assert len(confirm_rows) == 1
    assert confirm_rows[0]["command"] == "B2:B4 값 지워줘"
    assert confirm_rows[0]["approved"] is None
    ...
    assert second.status_code == 200
    assert audit.updated == [(confirm_rows[0]["id"], True)]
    # 재개 실행은 CONFIRM 행 갱신으로 끝난다 — SAFE로 또 남기면 표에 두 줄이 된다.
    assert [r["grade"] for r in audit.logged].count("SAFE") == 0


def test_rejected_approval_marks_row_false(monkeypatch):
    monkeypatch.setattr(excel_live_router, "get_excel_live_service", _one_fake())
    audit = _patch_cmd_audit(monkeypatch)
    excel_live_router._pending_operation_slots.clear()
    ...
    assert resp.status_code == 200
    row_id = [r for r in audit.logged if r["grade"] == "CONFIRM"][0]["id"]
    assert audit.updated == [(row_id, False)]


def test_safe_read_command_logs_safe_once(monkeypatch):
    """읽기(SAFE) 실행 턴은 SAFE 1행 — 잡담·되묻기는 기록하지 않는다는 규칙의 양성 쪽."""
    monkeypatch.setattr(excel_live_router, "get_excel_live_service", _one_fake())
    audit = _patch_cmd_audit(monkeypatch)
    excel_live_router._pending_operation_slots.clear()
    ...
    resp = client.post(
        "/excel-live/command",
        json={
            "message": "A1:C3 범위 읽어줘",
            "workbook_id": r"C:\work\sales.xlsx",
            "sheet_name": "Sheet1",
            "session_id": "sess-audit-safe",
            "approve": False,
        },
        headers=HEADERS,
    )
