...
_SW_VERB = (
    # 어간 단독(적·넣·쓰·채우)은 두지 않는다 — "H1 2026년 실**적**"의 끝 글자를
    # 동사로 먹어 셀에 '2026년 실'이 써졌다(2026-08-20 자체 검토 실측).
    # 어간이 필요했던 꼴(적으세요·넣으세요·쓰세요·채우세요)은 명시형으로 받는다.
    r"(?:입력|기입|기록|작성"
    r"|써|쓰세요|적어|적으세요|넣어|넣으세요|채워|채우세요"
    r"|write|set|input)"
...
_COMMON_TYPO_PAIRS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (re.compile(r"정열"), "정렬"),
    (re.compile(r"함계"), "합계"),
    # 285턴 강건성 코퍼스 실측(2026-09-06): 아래 오타가 규칙·플래너 양쪽을 빗나가게 했다.
    (re.compile(r"판메량"), "판매량"),
    # '판매랑'은 조사(판매랑 재고랑)일 수 있어 정렬·비교 문맥에서만 고친다.
    (re.compile(r"판매랑(?=\s*(?:많|적|높|낮|큰|작|순|기준))"), "판매량"),
    (re.compile(r"막데(?=\s*(?:차트|그래프))"), "막대"),
    (re.compile(r"메출"), "매출"),
    ...
    (
        re.compile(
            ...
            # `병합 셀에`는 병합**된** 셀을 가리키는 명사구다 — "제품코드 X1Y2 병합 셀에
            # 넣어줘"의 X1Y2는 값이지 범위가 아니다(2026-08-20 자체 검토: X1:Y2로 변조됐다).
            r"(?=\s*(?:병합|merge|합쳐|합치|한\s*칸으로)(?!\s*(?:셀|칸)\s*에))"
        ),
        r"\1:\2",
    ),
    (re.compile(r"합게"), "합계"),
    ...
    # "차트 다 지어줘" — 지우다의 오타. 다른 해석이 없는데도 되묻기 종류가
    # 턴마다 흔들렸다(GUI 조건 ×3 결정성 게이트 실측). 결정적으로 정규화한다.
    (
        re.compile(
            r"((?:차트|그래프|셀|값|내용|서식|테두리|배경|시트|표|행|열|전부|다|싹|전체)\s*)지어\s*(줘|주라|주세요)"
        ),
        r"\1지워\2",
    ),
...
_ROW_WRITE_PATTERN = re.compile(
    ...
    r"\b([a-z]+\d+:[a-z]+\d+)\s*(?:여기(?:에다가|에다|다가|다|에)?|이\s*(?:범위|영역|자리|곳|위치|지점)에?|에(?:다가?)?)\s*((?:[^\n]|\n)+?)\s*"
...
_ROW_WRITE_TRAILING_DEICTIC = re.compile(
    ...
    r"(?:(?:이거|이걸|이것|요거|이\s*값|이\s*내용)\s*)?(?:여기(?:에다가|에다|다가|다|에)?|요기에?|이\s*(?:범위|영역|자리|곳|위치|지점)에?)\s*"
...
_PASTE_LOCATIVE_LEAD = re.compile(
    ...
    # "지금 이 위치에 …" — '위치'가 자리말 명사 목록에 없고 앞의 '지금'도 안 벗겨져, 문장 전체가
    # 규칙을 빗나가 플래너로 갔다(2026-09-06 GUI 실사고: GPU 없는 PC라 플래너까지 타임아웃 →
    # "개수 세기로 이해했어요"). 시간 부사(지금·현재·방금)와 위치·지점·커서 위치를 받는다.
    + r"|(?:(?:지금|현재|방금|바로)\s*)?(?:이|그|저|요|바로|그\s*다음|다음|새|새로운)\s*(?:곳|쪽|자리|위치|지점|범위|영역|칸|셀|표|것|거|줄|행|열|시트)\s*" + _PASTE_POS + r"?\s*" + _PASTE_PARTICLE + r"?"
    + r"|(?:지금|현재|방금)\s*(?:위치|자리|커서\s*위치|선택\s*위치)\s*" + _PASTE_PARTICLE + r"?"
    + r"|커서\s*(?:위치|자리)\s*" + _PASTE_PARTICLE + r"?"
...
# 붙여넣기 뒤의 쓰기 동사 — 사람은 "입력해바"·"적어 주세요"·"paste 해 주세요"·"input 부탁드려요"라고도 쓴다.
# 좁게 잡으면 문장이 통째로 규칙을 빗나가 값이 한 칸에 박히거나 되묻기로 샌다(2026-08-20 게이트 8건).
_PASTE_WRITE_VERB_TAIL = re.compile(
    r"(?:값|데이터|내용|이거|이걸|이렇게)?\s*(?:을|를)?\s*"
    r"(입력|기록|넣어|넣|채워|채우|써|적어|쓰|paste|input|붙여\s*넣기?)"
    r"\s*(?:해|하)?\s*"
    r"(?:줘요|줘|주세요|주새요|주라|주시|줄래|주실\s*수\s*있(?:을까요|나요)|주시겠어요"
    r"|주시면\s*\S{0,12}|놔|둬|봐|바|부탁\s*드려요|부탁해요|부탁해|부탁드립니다|부탁)?"
    r"\s*(?:도\s*될까요|도\s*괜찮(?:을까요|나요))?"
    r"\s*(?:그대루|그대로|순서대로|차례대로)?\s*[~.!?…]*\s*$",
    re.IGNORECASE,
)
...
def parse_rangeless_row_write(text: str, target_range: str) -> dict | None:
    ...
    # "…,30 **이어서** 넣어줘" — 이어 붙이라는 말도 아래쪽 배치다. 머리말이 아니라
    # **값 나열 뒤 꼬리**에 붙으므로 _head_zone 으로는 영영 안 보인다.
    # 이걸 놓치면 잡힌 범위의 좌상단부터 덮어써 **사람이 친 머리글과 첫 행이 사라진다**
    # (2026-09-08 시드 배터리 실측: 제품·카테고리… 머리글이 통째로 날아갔다).
    _tail_zone = re.split(r"[,;\n]", source)[-1]
    if re.search(r"(?:이어서|이어|계속해서|추가로|덧붙여)", _tail_zone):
        _place_below = True
    ...
    # "지역별 데이터 두 줄을 추가해야 **해서요,** 지금 붙여넣은 자리에 …" — 상황 설명 머리말은 값이 아니다.
    # 안 떼면 문장 전체가 첫 칸에 들어간다(2026-08-20 블라인드 게이트 paste_values).
    body = re.sub(r"^[^,;\n]{0,60}?(?:해서요|하셔서요|해서|라고\s*하셔서|때문에|하는데|인데)\s*,\s*", "", body)
    body = re.sub(r"^(?:[^\s,]+\s+){0,2}[^\s,]+\s*(?:시트|탭)(?:에|의|에서|에다가?)?\s*", "", body)
    # "방금 붙여넣은 표 아래에 …", "선택한 범위에 …" — 자리를 가리키는 말머리도 값이 아니다.
    body = re.sub(
        r"^(?:(?:방금|지금|아까|막)\s*)?(?:붙여\s*넣은|붙여\s*넣기\s*한|복사한|복붙한|선택한|선택|잡은|드래그한|고른)\s*"
        r"(?:(?:한|두|세|네|다섯|여섯|\d{1,3})\s*)?"
        r"(?:표|범위|영역|곳|자리|칸|칸들|셀|셀들|줄|행|줄들)?\s*(?:아래에?|밑에?|에다가?|에|의|들에)?\s*",
        "",
        body,
    )
    ...
    body = re.sub(
        # `이어서`류를 빼지 않으면 마지막 칸이 '30 이어서'로 오염된다(2026-09-08 실측).
        r"(?:\s*(?:순서대로|차례대로|순으로|각각|전부|모두|이렇게|요렇게|이거|이것|요거|여기에?|여기다"
        r"|이어서|이어|계속해서|추가로|덧붙여|으로|로|을|를|붙여서|붙여|대로|그대로|한\s*번에|쭉))+\s*$",
        "",
        body,
    )
