...
뒀는데, 그 기준으로 당시 `logs/all_events.jsonl` 10,827건을 재분류하니 5,844건이
사람으로 나왔다(그 파일은 2026-09-10 부터 없다 — 이벤트는 `chat_log.jsonl` 의
`record: event` 줄이고, `decision_trace.iter_records(record="event")` 로 읽는다). 그런데 그 5,844건은 요청 간격 중앙값이 1.7초였고, 4,338건이
