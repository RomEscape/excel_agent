    ...
    def create_table(
        ...
        if with_border:
            ...
            if api_range is not None:
                # 플랫폼 분기(COM/AppleScript)는 excel_border가 전부 흡수한다 —
                # 예전 COM 직접 호출은 dev 병합(2026-08-29)으로 _rgb_to_excel_color가
                # 사라지면서 NameError가 됐다.
                try:
                    excel_border.apply_borders(
                        api_range, line_style="continuous", weight="thin", rgb=(0, 0, 0)
                    )
                except excel_border.BorderUnsupportedError:
                    pass  # 표는 이미 만들어졌다 — 테두리만 못 그린 것뿐이다.
