...
class PendingExcelApproval:
    ...
    # 사용자가 실제로 입력한 자연어 명령. `작업 기록` 표의 `명령` 칸에 그대로 나간다.
    command_text: str = ""
...
def _intent_first_enabled() -> bool:
    """통역 AI(의도 정규화)를 규칙표보다 앞에 세우는 스위치.

    로드맵 2단계. 2026-09-01 까지 켠 것과 끈 것을 견줘 파괴 게이트 72/72 · 대화 전록
    421/421 · 말투 게이트 604=604 로 전 지표가 같았고, 2026-09-10 사용자 지시로
    **기본을 켜짐**으로 바꿨다. 끄려면 `OFFICECLAW_INTENT_FIRST=0`(또는 false/no/off).
    환경변수를 매번 읽는다(캐시하지 않는다): 한 프로세스 안에서 켜고 끄며
    A/B를 돌릴 수 있어야 하고, 이 판정은 턴당 한 번이라 비용이 문제가 되지 않는다.
    """
    raw = str(os.environ.get("OFFICECLAW_INTENT_FIRST", "")).strip().lower()
    if not raw:
        return True
...
async def _run_command(
    ...
    # ── 통역 AI(의도 정규화)를 규칙표보다 **앞**에 세운다(로드맵 2단계) ──────
    # 2026-09-10 부터 기본 켜짐(`OFFICECLAW_INTENT_FIRST=0` 으로 끈다).
    #
    # 처음(2026-08-23)엔 624 게이트 96.3% 중 규칙 경로가 492건이고 통역의 정확도가
    # 79%라 뒤집으면 나빠질 수 있어 스위치로 빼고 꺼 둔 채 쟀다. 2026-09-01 까지
    # 켠 것과 끈 것이 파괴 72/72 · 전록 421/421 · 말투 604=604 로 같아졌고, 켠 채
    # 파괴 게이트 72/72(2026-09-10_2324) 를 확인한 뒤 기본을 켰다.
    # 아직 이 블록 **뒤**에서 `title_row_merge`·`clear_scope` 같은 축소 규칙이
    # `should_parse_with_llm` 을 끄고 모델을 건너뛴다(파괴 게이트 72턴 중 22턴) —
    # 그 규칙들을 모델 뒤의 범위 가드로 옮기는 것이 다음 단계다.
    if (
...
_UNSTATED_PARAM_RULES: tuple[tuple[frozenset[str], re.Pattern[str], str], ...] = (
    (
        ...
        re.compile(
            ...
            # "등수 매기게 점수 순으로" — 순위를 매긴다는 말은 큰 값부터라는 뜻이다.
            # 이걸 방향 없음으로 보면 사람이 이미 말한 것을 다시 묻는다(2026-08-27 실측).
            r"아래로|역순|거꾸로|가나다|ㄱㄴㄷ|최신|최근|오래된|예전|빠른|늦은|차순"
            r"|등수|순위|랭킹|석차|top\s*\d)",
            re.IGNORECASE,
        ),
...
def _plan_approval_gate(
    ...
    for step in plan:
        ...
        if tool_def and tool_def.permission == PermissionLevel.DENIED:
            ...
            _log_command(
                grade="DENIED",
                command=str(getattr(req, "message", "") or ""),
                reason="보안 정책에 의해 거부된 작업입니다.",
    ...
    _pending_approvals[pending.approval_id] = PendingExcelApproval(
        ...
        audit_id=audit_id or None,
        command_text=str(getattr(req, "message", "") or ""),
    )
    trace_route(
        "approval:required",
...
async def _execute_plan_and_respond(
    ...
    # 실행이 성공으로 판정된 턴을 `작업 기록`(command_log)에 남긴다. 되묻기·잡담
    # 턴은 단계가 없어 걸리지 않는다 — 작업 기록은 대화록이 아니라 작업 목록이다.
    if log_safe and execution.steps:
        _log_command(
            grade="SAFE",
            command=str(getattr(ctx.req, "message", "") or ""),
            reason="자동 실행 (허용된 작업)",
...
async def post_approval(
    ...
    with trace_origin(
        ...
        if getattr(pending, "audit_id", None) is not None:
            get_command_audit_logger().update_approval(pending.audit_id, bool(req.approved))
        ...
        if pending.resume is not None:
            ...
            # 이 명령은 이미 CONFIRM 행으로 남아 있고 위에서 승인 결과로 갱신됐다 —
            # 여기서 또 남기면 같은 명령이 표에 두 줄로 보인다.
            response = await _execute_plan_and_respond(pending.resume, llm, log_safe=False)
            set_outcome_from_response(response)
            return response
