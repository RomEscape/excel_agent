...
- **제목에 한국시간(KST)을 함께 적는다.** 하루에 항목이 여러 개 쌓이면 날짜만으로는
  어느 것이 나중 것인지 알 수 없다 — 앞 항목의 "남은 것"을 뒤 항목이 닫았는지 판별하려면
  순서가 필요하다. 형식은 `## YYYY-MM-DD (요일) HH:MM KST — 제목`.
  > **함정**: 이 개발기의 Git Bash 에서 `TZ=Asia/Seoul date` 는 KST 가 아니라 **UTC** 를
  > 뱉는다(2026-09-06 실측: 로컬 22:40 인데 13:40 출력). 윈도우 시간대가 이미 KST 이므로
  > 그냥 `date` 를 쓰거나, PowerShell 의 `Get-Date` 를 쓴다. 지난 항목의 시각을 되살릴 때는
  > 지어내지 말고 `git blame --line-porcelain -- 개발일지.md` 의 author 시각을 쓴다.
...
**새 말을 지어내지 않는다.** 아래 표에 있으면 그 말을 쓰고, 없는 개념이면 **표에 먼저
한 줄을 추가한 뒤** 쓴다. 이름이 갈리면 `grep` 이 안 먹고, 반년 전 기록과 오늘 기록이
같은 것을 말하는지 알 수 없게 된다(2026-09-07 실측: 같은 개념이 두 이름으로 불린
사례 3건, 정의 없이 쓰인 말 1건).

| 표준어 | 뜻 | 쓰지 말 것 |
|---|---|---|
| **미검출 오실행** | 시스템은 성공이라 보고했는데 실제로는 잘못 실행된 것. `silent_max`가 세는 값이며 **0에서 절대 올리지 않는다**. | 조용한 오실행 |
| **되묻기** | 추측해서 실행하는 대신 사용자에게 물어보는 것(`excel_live.clarify`). | 재질문 |
| **승인 카드** | 실행 전에 사용자 확인을 받는 화면 전반. | — |
| **해석 카드** | 승인 카드 중, 모델이 해석한 계획을 "이렇게 이해했어요"로 보여 주는 것. **승인 카드의 한 종류다.** | — |
| **블라스트 반경** | 쓰기가 사용자가 지목하지 않은 자리의 값까지 덮는 범위(`excel_write_scope`). | 폭발 반경 |
| **빠른 규칙** | LLM 없이 규칙만으로 계획을 확정하는 경로. 로그에는 `quick_rule:hit`/`:miss`로 찍힌다. | — |
| **액션 별칭 정규화** | 같은 데이터 효과를 내는데 이름만 둘인 액션(`sort_rows`≡`sort_range`)을 **채점할 때만** 한 이름으로 묶는 것. 실행 경로는 건드리지 않는다. | 동의어 접기, 동의어 라벨 통합 |
| **플래너** | Excel 계획 JSON 전용 SFT 모델(`ax7bplanner-*`). 일반 대화·고수준 분해를 맡는 `model`과 구분한다. | — |
| **비교 조건** | A/B 측정에서 서로 견주는 두 벌의 실행. `스킬 없이`·`스킬 넣고` 처럼 **무엇이 다른지를 그대로** 적는다. | 팔, A팔/B팔, arm |
| **파괴 게이트** | 위험한 명령 72개를 자동으로 넣어 잘못 실행되는지 보는 야간 검사. `silent_max`(미검출 오실행)가 여기서 나온다. | — |
| **말투 게이트** | 같은 뜻을 다른 말투로 바꿔 쓴 624문장으로 일반화를 보는 야간 검사. | — |
| **대화 배터리** | 사람과 여러 턴 주고받는 각본을 GUI와 같은 순서로 재현하는 검사(`run_dialogue.py`). | — |
| **실제로 바뀐 셀** | 명령 전후로 통합문서를 열어 대조해 값·서식이 달라진 칸. `excel_workbook_diff` 가 낸다. 판정을 응답의 자기보고가 아니라 이걸로 한다. | 발자국 |
| **채점 기준** | 배터리·게이트가 한 턴을 합격·불합격으로 정하는 규칙. | 눈금 |
| **무실행** | `expect=ok` 인데 통합문서가 하나도 안 바뀐 것. 시스템은 성공이라 보고했다. **미검출 오실행**과 다르다 — 그쪽은 엉뚱한 데를 바꾼 것이다. | — |
| **전면 보류** | "지금은 아무것도 하지 마"처럼 문장 전체가 실행 중단인 것. 일부만 남겨 두라는 **범위 한정**("나머지는 그대로 둬")과 다르다. | — |

> 표에 없는 말을 쓰고 싶으면 그 자리에서 만들지 말고 **여기 한 줄을 먼저 추가**한다.
> 개발일지·커밋 메시지·코드 주석 모두 이 표를 따른다.

### 쓰지 않는 표현

- **"팔"·"A팔/B팔"** — 실험 비교군을 이렇게 부르지 않는다. 무엇이 다른지를 그대로 쓴다
  (`스킬 없이 돌린 것` / `스킬 넣고 돌린 것`).
- **"온도"·"같은 온도"** — 모델 샘플링 설정을 이런 말로 설명하지 않는다. 필요하면
  "같은 입력에 같은 답을 내도록 고정돼 있다"처럼 **무슨 일이 일어나는지**로 적는다.
- **"발자국"** — 실무 용어가 아니다. `실제로 바뀐 셀` 로 쓴다. 계획이 쓸 범위를 뜻할
  때는 `쓰기 범위`(`write_footprint` 가 내는 것).
- **"눈금"** — 실무 용어가 아니다. `채점 기준` 으로 쓴다. "눈금이 멀다" 같은 비유도
  쓰지 않는다 — "검사가 무력화됐다" 처럼 무슨 일이 일어났는지로 적는다.
- **"고아가 된다"** — 정리되지 않고 남은 옛 프로세스를 이렇게 부르지 않는다.

> 2026-09-10 사용자 지적: "실무에서 사용하는 용어도 아니고 도무지 무슨 소리인지
> 모르겠어 내가 해석까지 해야겠니?" — 내가 `발자국`·`눈금` 을 정의 없이 지어내
> 18곳에 썼다. §6 첫 줄("새 말을 지어내지 않는다")을 어긴 것이다.
> **읽는 사람이 해석해야 하는 말은 쓰지 않는다.**
