...
셋업 스크립트 하나가 도구(Node·Rust·uv·Ollama)·의존성·모델까지 전부 준비한다.
...
5. **알려진 macOS 한계**(P2 이식 후): 라이브(엑셀 앱 제어) 모드에서
   **조건부 서식 3종(데이터 막대·색조·수식 CF)과 입력 유효성 검사만** 불가 —
   macOS Excel의 AppleScript 자동화에 해당 API가 없다(파일 엔진은 지원).
   그 외 전 액션 — 틀 고정·표 변환·시트 보호·인쇄·셀 메모·VBA·새로고침 포함 —
   은 xlwings/_xlmac 관용구 기준으로 구현돼 있다(**실기 검증 전**: 첫 맥 구동에서
   어긋나는 항목은 명확한 오류로 나오며, 개발일지에 실측으로 남겨 달라).

## 진단·감사 보고서 — 발행된 아티팩트 (claude.ai 링크)

> 2026-08-29 dev 리베이스 브리지 커밋에서 유실됐다가 2026-09-01 복원한 표다.
