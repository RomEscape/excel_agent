...
5. **알려진 macOS 한계**(P1 이식 후, 코드가 명확한 오류로 거절하는 것만):
   조건부 서식(데이터 막대·색조·수식 CF)·틀 고정·표 변환·시트 보호·유효성 검사·
   셀 메모·인쇄 설정·VBA 매크로·Power Query. 그 외 — 값·수식·정렬·테두리·색칠·
   글꼴·병합·차트 생성/삭제·중복 제거·행/열 삭제·수식 열 채움 — 는 고수준
   크로스플랫폼 API로 동작하도록 설계돼 있다(실기 검증 전).
