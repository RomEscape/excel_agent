...
async def main() -> None:
    ...
    parser.add_argument(
        "--scenario-pack",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "datasets" / "excel_demo_scenarios_v1.json",
    )
    parser.add_argument("--scenario-id", type=str, default="")
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "logs" / "demo_com_artifacts",
    )
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "logs" / "excel_demo_com_report.json",
    )
