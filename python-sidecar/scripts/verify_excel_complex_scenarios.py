...
def _seed_from_template(path: Path, template: Path) -> None:
    ...
    """
    resolved = template if template.is_absolute() else (Path(__file__).resolve().parents[2] / template)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--scenario-pack",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "datasets" / "excel_complex_scenarios_v1.json",
    )
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "logs" / "excel_complex_verify_report.json",
    )
