...
def test_chat_uses_llm_fallback_when_gateway_yields_no_content(monkeypatch):
...
def test_chat_falls_back_to_default_when_llm_fallback_fails(monkeypatch):
    """LLM 보강도 실패하면 기존 안전 기본값을 유지한다."""
...
def test_chat_uses_llm_fallback_on_openclaw_degraded_reply(monkeypatch):
    """OpenClaw가 context overflow류 문구를 주면 직접 LLM 응답으로 대체한다."""
    fake = _FakeClient(
        frames=[
            {
