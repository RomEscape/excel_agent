    ...
    *,
    ...
    if flags.get("workbook_read_only") and name not in READ_ONLY_SAFE_ACTIONS:
        return WriteBlock(
            ...
            # 문구가 실제로 통하는 길을 알려 줘야 한다. 2026-08-16 실측에서 이 PC의
            # Excel은 정품 인증이 안 된 무료 버전이라 여는 통합문서가 **전부** 읽기
            # 전용이었다 — "읽기 전용 해제"만 안내하면 사용자는 빠져나갈 수 없다.
            # Excel을 닫으면 file 엔진(openpyxl)이 파일을 직접 편집할 수 있다.
            reason=(
                "이 통합문서는 읽기 전용으로 열려 있어 Excel을 통해서는 편집할 수 없습니다.\n"
                "다음 중 하나로 풀 수 있습니다:\n"
                "1) Excel을 닫아 주세요 — 그러면 제가 파일을 직접 편집합니다 (가장 빠릅니다).\n"
                "2) Excel 상단에 '편집 사용' 배너가 있으면 눌러 주세요.\n"
                "3) 무료 버전 Excel이면 파일을 OneDrive에 저장하거나 정품 인증이 필요합니다."
            ),
            detail=dict(flags),
        )
