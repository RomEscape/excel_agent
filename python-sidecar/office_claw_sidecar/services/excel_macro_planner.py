...
# "대시보드 시트", "대시보드라는 시트", "대시보드란 이름의 탭", "대시보드(이)라고 시트" — 전부 시트 이름이다
# (2026-08-19 ex12 v2 실측: "대시보드라는 시트 하나 더 만들어주세요"가 24단계 매크로로 분해됐다).
_SHEET_NAMING_SUFFIX = re.compile(
    r"^(?:(?:이?라는|이?란|이?라고|이름(?:의|으로|은)?|명으로|용)\s*(?:이름(?:의|으로|은)?\s*)?)?(시트|탭|sheet|tab)",
    re.IGNORECASE,
)
...
def build_macro_prompt(
    ...
    *,
    ...
    return (
        ...
        # 예시를 A1:F2로 두었더니 모델이 규칙 대신 이 리터럴을 베꼈다(2026-08-16 실측).
        # ax4-light가 데이터 시트에 "배송_데이터 시트 A1:F2 병합해줘"를 냈다 — 예시와
        # 바이트 단위로 같다. 두 줄 병합은 머리글+첫 데이터행을 삼킨다. 한 줄로 줄이고
        # 대상이 새로 만든 시트임을 예시 자체에 박는다.
        "     - 제목 병합(새로 만든 시트의 1행만): \"<새시트> 시트 A1:F1 병합해줘\"\n"
