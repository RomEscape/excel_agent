...
_AGG_WORD = (
    r"(합계|총합계|총합|총계|합산|평균|개수|합"
    r"|다\s*더한\s*값|전부\s*더한\s*값|더한\s*값|전부\s*더해서|다\s*더해서|더해서"
    # "전부 **합쳐서**", "다 **합친** 값" — 사람이 '더하다'만큼 자주 쓴다.
    # 이게 없어 "성적부에 있는 결석 수 전부 합쳐서 A2에"가 규칙 밖으로 샜다(2026-08-20).
    r"|전부\s*합쳐서|다\s*합쳐서|합쳐서|합친\s*값|합쳐|합해서)"
)
...
_CROSS_SHEET_CELL_LAST = re.compile(
    ...
    + r"\s*(?:을|를|값을|값)?\s*(?:여기\s*)?(?:[^\s,]{1,20}\s+){0,2}"
