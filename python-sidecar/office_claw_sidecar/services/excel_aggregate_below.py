...
# **긴 것부터 적는다.** 정규식 교대는 순서대로 시도되므로 `합`이 앞에 있으면
# "합친 숫자"가 `합`으로 잘리고 머리글 자리에 '전부' 같은 부사가 들어온다
# (2026-08-20 624 게이트: `전부 합친 숫자`가 이렇게 깨졌다).
_AGG_WORD = (
    r"(총합계|총합|총계|합계|합산"
    r"|다\s*더한\s*(?:값|숫자)|전부\s*더한\s*(?:값|숫자)|더한\s*(?:값|숫자)"
    r"|전부\s*더해서|다\s*더해서|더해서"
    # "전부 **합쳐서**", "다 **합친** 값/숫자" — 사람이 '더하다'만큼 자주 쓴다.
    r"|전부\s*합쳐서|다\s*합쳐서|합쳐서|합해서"
    r"|합친\s*(?:값|숫자|것)|합한\s*(?:값|숫자|것)|합쳐"
    r"|평균|개수"
    # 영어로 부르는 사람도 있다 — "주문건수 total 넣어 주세요".
    r"|total|sum|average|count"
    # 짧은 `합`은 **맨 마지막**이다. 위의 어떤 것도 아닐 때만 걸려야 한다.
    r"|합)"
)
...
_CROSS_SHEET = re.compile(
    r"([A-Z]+\d+)\s*(?:셀|칸)?\s*(?:에(?:는|다가|다)?)?\s*(?:([^\s,]+?)\s*(?:시트|탭|sheet)(?:의|에서|에\s*있는|안에\s*있는|안의|에)?\s*)?"
...
_CROSS_SHEET_CELL_LAST = re.compile(
    r"(?:([^\s,]+?)\s*(?:시트|탭|sheet)(?:의|에서|에\s*있는|안에\s*있는|안의|에)?\s*)?"
    # 시트 낱말 없이 이름만 부르고 **출처 조사**를 붙인 꼴("성적부**의** 결석 합계",
    # "성적부**에 있는** 결석 수"). 시트 이름은 아래 폴백이 문장에서 다시 찾으므로
    # 여기서는 자리만 비켜 준다 — 그룹 번호를 흔들지 않으려고 캡처하지 않는다.
    r"(?:[^\s,]{1,20}(?:의|에서|에\s*있는|안에\s*있는|안의)\s+)?"
    + _HEADER_TOKEN
    + r"\s*(?:의|을|를)?\s*"
    + _AGG_WORD
    # 집계어와 대상 셀 사이에 말이 낀다: "합계를 **요약** A2에", "합쳐서 **가져와서** A2에".
    # 두 낱말까지 허용한다(쉼표는 못 넘는다 — 다른 절로 새지 않게).
    #
    # 예외로 **쉼표 뒤 대명사** 하나는 넘는다: "…합계**, 그걸** A2에 넣어 주세요".
    # 대명사가 앞 절의 집계를 그대로 받으므로 다른 절로 새는 게 아니다
    # (2026-08-20 624 게이트에서 이 문형이 규칙 밖으로 샜다).
    + r"\s*(?:을|를|값을|값)?\s*(?:\s*,\s*(?:그걸|그거|그것을|그것|이걸|이거|이것을|그\s*값을?)\s*)?"
    + r"(?:여기\s*)?(?:[^\s,]{1,20}\s+){0,2}"
    + r"([A-Z]+\d+)\s*(?:셀|칸)?\s*(?:에|로|으로|에다가?|다)?",
    re.IGNORECASE,
)
# 머리글이 될 수 없는 수량·정도 부사. 이게 머리글 자리에 오면 진짜 머리글은 앞 토큰이다.
_QUANTIFIER_ONLY = re.compile(r"(?:다|전부|모두|싹|전|모|총|일괄|죄다|몽땅|통째로?)")
#: "…해 줘"에 해당하는 동사들. `보여`가 없어 "합친 숫자 **보여줘**"가 규칙 밖으로 샜다.
_CROSS_SHEET_VERB = re.compile(
    r"가져|끌어|연결|수식|넣어|채워|기록|불러|참조|더해|계산|놔|놓아|써|입력|보여|보이|표시"
)
