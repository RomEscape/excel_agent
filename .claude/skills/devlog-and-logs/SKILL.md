...
- [ ] 날짜는 절대 표기("어제" 금지)
- [ ] **제목에 한국시간** — `## YYYY-MM-DD (요일) HH:MM KST — 제목`.
      하루에 항목이 여러 개 쌓이면 날짜만으로는 순서를 알 수 없다(2026-09-06 실측: 하루 17개).
      **셸의 `TZ=Asia/Seoul date` 는 이 개발기에서 UTC 를 뱉는다** — 그냥 `date` 나
...
> `lefthook`의 `devlog-guard`가 커밋을 막아 준다. 파일이 바뀌었는지에 더해,
> **새로 추가된 제목에 `HH:MM KST` 가 없으면 종료코드 1로 막고 지금의 KST 를 찍어 준다**
> (2026-09-06 추가, `scripts/check-devlog-update.mjs`). 다만 **내용이 실측인지는
> 사람만 안다** — 훅은 형식만 본다.
